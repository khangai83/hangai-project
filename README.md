...
#### 🎯 Анхаарал татах дизайн (шинэчлэгдсэн)

Товч нь бусад toolbar элементээс **мэдэгдэхүйц ялгагдана** — ингэснээр хэрэглэгч
«Дэлгэрэнгүй хайлт» байгааг анзаарахгүй өнгөрөхгүй:

| Элемент | Шийдэл |
|---|---|
| Хэлбэр | `rounded-full` (дугуй pill) + `px-4 py-2.5` — илүү том |
| Биет | **Градиент** `from-[#3b82f6] to-[#1d4ed8]` + цагаан `font-bold` текст |
| Сүүдэр | `shadow-btn-primary` (товчны нэгдсэн 3D систем) |
| **Гэрэлтэлт (glow)** | Ард нь `absolute -inset-[3px] blur-[7px]` **цайвар цэнхэр halo** — шүүлт идэвхтэй үед `bg-primary/25 → /45` болж хүчтэйрнэ |
| Icon | `⚙️` нь `bg-white/20` дугуй дотор (градиент дээр цэвэрхэн харагдана) |
| Тоо | **ЦАГААН дугуй** дотор цэнхэр тоо — урьд нь эсрэгээрээ (цэнхэр дотор цагаан) байсан тул градиент дээр ялгагдахгүй байв |
| Хөдөлгөөн | hover → `-translate-y-0.5` + сүүдэр томорно; `▼` 180° эргэлдэнэ |

#### Нээгдсэн панель

```
┌─────────────────────────────────────────────────────┐
│ ⚙️ Дэлгэрэнгүй хайлт  [ 3 шүүлт ]            ✕ Хаах │  ← primary/5 дэвсгэртэй толгой
├─────────────────────────────────────────────────────┤
│  Өрөө        │  Хот/Аймаг  │  Дүүрэг    │  Хороо    │
│  Үнэ (доод)  │  Үнэ (дээд) │  Талбай…   │           │
└─────────────────────────────────────────────────────┘
```

- Хүрээ нь `border-2 border-primary/25` + `shadow-card-hover` — нээлттэй үед тод ялгарна
- Толгой нь `bg-primary/5` дэвсгэртэй зурвас + **«N шүүлт»** badge
- `✕ Хаах` товч нь hover дээр бүрэн цэнхэр болно
- Толгой нь сөрөг margin (`-mx-5 -mt-5`) ашигладаг — ингэснээр доорх форм-ыг дахин
  бүтэцлэхгүйгээр container-ийн padding дээгүүр гарч, будагдсан зурвас үүснэ
