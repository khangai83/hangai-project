...
// 0003_listing_details.sql / 0011_listing_video.sql-ээр нэмэгдэх баганууд
// (migration ажилласан эсэхийг таних — байхгүй бол тэдгээрийг орхиод хадгална)
const DETAIL_COLUMNS = ['build_year', 'floor', 'total_floors', 'balconies', 'has_garage', 'video_url'];
...
/** Дэлгэрэнгүй мэдээллийн баганын нэрс (0003 + 0011) */
const DETAIL_ROW_KEYS = ['build_year', 'floor', 'total_floors', 'balconies', 'has_garage', 'video_url'];
  ...
  if (error) {
    ...
    if (isMissingDetailColumnError(error)) {
      console.warn(
        ...
        '(build_year/floor/total_floors/balconies/has_garage/video_url). ' +
        'supabase/migrations/0003_listing_details.sql ба 0011_listing_video.sql-ийг ' +
        'Supabase SQL Editor-т ажиллуулна уу. ' +
        'Одоогоор нэмэлт мэдээлэл (видео линк орсон) хадгалагдсангүй.'
      );
...
export async function updateListing(userId, listingId, payload) {
  ...
  const row = listingPayloadToRow(payload);
  // ⚠️ Нэмэлт багана (0003/0011) байхгүй нь тогтоогдвол `true` болно —
  //    доорх DELETE+INSERT fallback-д ТЭР БАГАНУУДЫГ ХАСАХ ёстой, эс бөгөөд
  //    хуучин зарыг устгачихаад шинэ мөр оруулж чадахгүй → ЗАР АЛГА БОЛНО.
  let detailColumnsMissing = false;
  ...
  // Нэмэлт багана (0003/0011) байхгүй бол суурь талбаруудаар дахин оролдоно
  if (isMissingDetailColumnError(direct.error)) {
    detailColumnsMissing = true;
    const base = { ...row };
  ...
  // ⚠️ `video_url` (эсвэл 0003-ын багана) байхгүй нь мэдэгдсэн бол тэдгээрийг
  //    ХАСАЖ оруулна — эс бөгөөд INSERT нурж, зар алга болно (дээрх тайлбар).
  const fallbackRow = { ...row };
  if (detailColumnsMissing) DETAIL_ROW_KEYS.forEach((k) => delete fallbackRow[k]);
