...
/**
 * «Үзсэн» тоог +1 (detal хуудас нээгдэх бүрд).
 * ⚠️ Алдааг ШИДЭХГҮЙ — тоолуур нь косметик, зар харахад саад болохгүй.
 *    Ажиллаагүй/миграцгүй бол `null` буцаана.
 * @returns {Promise<number|null>} шинэ «үзсэн» тоо
 */
export async function trackListingView(id) {
  if (!id) return null;
  try {
    const res = await fetch(`/api/listings/${id}/view`, { method: 'POST', keepalive: true });
