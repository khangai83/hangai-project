...
/**
 * Зар засах (зөвхөн ӨӨРИЙН зар — RLS `listings_update` policy шаардана).
 * ⚠️ Хэрэв Supabase дээр update policy байхгүй бол UPDATE нь 0 мөр буцаана
 * (алдаа шидэхгүй) тул `.single()` нь PGRST116 алдаа өгнө — тэр үед ойлгомжтой
 * мессеж рүү хөрвүүлж өгнө.
 */
export async function updateListing(userId, listingId, payload) {
  ...
  const row = listingPayloadToRow(payload);
  const query = () =>
    sb.from('listings').update(row).eq('id', listingId).eq('user_id', userId).select().single();
  ...
  if (error) {
    if (isMissingDetailColumnError(error)) {
      ...
      const retry = await sb
        ...
        .select()
        .single();
      if (retry.error) throw updateError(retry.error);
      return retry.data;
    }
