...
export function buildHomeBreadcrumb({
  ...
  const items = [
    ...
    // «Үл хөдлөх» — категори ба төрлийг цэвэрлэнэ (байршил/үнэ хэвээр)
    {
      label: 'Үл хөдлөх',
      href: homeFilterHref({ category: 'all' }),
      nav: { category: 'all', filters: { propertyType: '', rooms: '' } },
    },
  ];
  ...
  if (cat !== 'all') {
    items.push({
      ...
      nav: { category: cat, filters: { propertyType: '', rooms: '' } },
    });
  }
