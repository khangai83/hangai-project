...
/** Санал хүсэлтийн жагсаалт + статистик (зөвхөн админд) */
export function fetchAdminFeedback() {
  return authedFetch('/api/admin/feedback');
}

/** Саналын төлөв / админы тэмдэглэл солих */
...
/** Зарын хайлт (ID / утас / нэр / дүүрэг ...) — зөвхөн админд */
export function fetchAdminListings(q = '', limit = 100) {
  const params = new URLSearchParams();
  if (q) params.set('q', q);
  if (limit) params.set('limit', String(limit));
  const qs = params.toString();
  return authedFetch(`/api/admin/listings${qs ? `?${qs}` : ''}`);
}

/** АДМИН — ЯМАР Ч зарыг устгах */
export function adminDeleteListing(id) {
  return authedFetch(`/api/admin/listings?id=${encodeURIComponent(id)}`, { method: 'DELETE' });
}
